# 健康数据分析指南

本文档为 LLM 提供健康数据分析的参考框架。

## 指标解读

### HRV（心率变异性）

- **含义**：反映自主神经系统状态，是恢复和压力的核心指标
- **正常范围**：个体差异大，20-100ms 均可正常
- **关键**：与个人基线对比，趋势比绝对值重要
- **高于基线**：恢复良好，副交感神经活跃
- **低于基线**：压力大、疲劳、疾病前兆、酒精影响等
- **连续 3 天低于基线 10%+**：需要关注

### 静息心率（RHR）

- **含义**：安静状态下的心率，反映心血管效率
- **正常范围**：40-100 bpm，运动员可能更低
- **低于基线**：心血管效率好，恢复好
- **高于基线**：疲劳、压力、疾病、过度训练
- **连续 3 天高于基线 5bpm+**：需关注

### 睡眠

| 指标 | 正常范围 | 过低含义 | 过高含义 |
|------|----------|----------|----------|
| 总时长 | 7-9 小时 | 睡眠不足 | 可能嗜睡 |
| 深睡占比 | 15-20% | 恢复不足 | — |
| REM 占比 | 20-25% | 记忆/情绪影响 | — |
| 醒来次数 | <3 次 | — | 睡眠中断 |
| 睡眠效率 | >85% | 入睡困难 | — |
| 入睡潜伏期 | 10-20 min | — | >30 min 可能失眠 |
| 时间一致性 | 7天入睡/起床时间标准差 | 标准差越小越好 | 波动大影响恢复 |

### 身体电量

- 0-100 量表
- >70：适合中高强度活动
- 30-70：适中，轻度活动
- <30：需要休息和恢复

### 压力

- 0-100 量表
- <40：低压力，放松状态为主
- 40-60：中等压力
- >60：高压力，需关注放松

### 血氧（SpO2）

- 正常：95-100%
- 93-95%：轻度偏低，关注呼吸
- <93%：建议就医
- ODI（氧减指数）：>5 次/小时需关注

### 训练负荷（心率储备法）

使用 Heart Rate Reserve (HRR) 方法，基于 WHO 推荐的每周 150-300 分钟中高强度运动。

| 区间 | 分钟/周 | 含义 |
|------|---------|------|
| inactive | 0 | 不活跃 |
| detraining | <75 | 训练不足 |
| low | 75-150 | 偏低 |
| productive | 150-300 | 有效提升（WHO 推荐） |
| overreaching | 300-450 | 负荷偏高 |
| high_risk | >450 | 过度训练风险 |

阈值计算：`threshold = RHR + (maxHR - RHR) × 0.45`（Karvonen 公式，45% 强度）

## 交叉分析模式

### 过度训练信号
- HRV 持续↓ + RHR 持续↑ + 睡眠质量↓ + 训练负荷↑
- 行动：立即安排 2-3 天主动恢复

### 疾病前兆
- HRV 突然大幅下降（>20%）+ RHR 升高 + 身体电量低
- 行动：注意休息，监测症状

### 酒精影响
- 深睡比例↓ + REM↓ + HRV↓ + RHR↑（饮酒后次日）
- 行动：减少饮酒，尤其睡前

### 时差/作息紊乱
- 睡眠效率↓ + 入睡时间↑ + HRV 波动大
- 行动：固定作息时间

### 恢复良好
- HRV 高于基线 + RHR 低于基线 + 身体电量高 + 睡眠质量好
- 行动：适合高强度训练

## 建议生成原则

1. **具体可执行**：「今晚 10 点前入睡」优于「注意睡眠」
2. **分级严重程度**：紧急（就医）、重要（立即调整）、一般（日常建议）
3. **不重复**：同一问题只提一次建议
4. **结合趋势**：单日异常可能是偶然，连续异常需要关注
5. **个性化**：根据用户运动习惯和目标调整建议
6. **正向鼓励**：状态好时给予正面反馈

## 输出结构模板

```
## 📊 今日数据概览
[关键指标摘要表格]

## 📈 7 天趋势
[趋势变化和异常标注]

## 🔍 综合分析
[多指标交叉分析]

## 💡 个性化建议
1. [最重要的建议]
2. [次要建议]
3. ...

## ✅ 今日行动
[根据当前状态的具体行动建议]
```
